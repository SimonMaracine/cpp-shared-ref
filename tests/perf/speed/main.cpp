...
template<typename SmartPointer, unsigned int Repeat>
double test_speed() {
...
int main(int argc, char** argv) {
    ...
    if (std::strcmp(arg, "ref")) {
        type = Type::Ref;
    } else if (std::strcmp(arg, "ptr")) {
        type = Type::Ptr;
    } else {
    ...
    switch (type) {
        case Type::Ref:
            result = test_speed<sm::shared_ref<Obj>, 100u>();
            break;
        case Type::Ptr:
            result = test_speed<std::shared_ptr<Obj>, 100u>();
            break;
    }
